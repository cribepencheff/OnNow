...
- "Open in [service]" for the show's Swedish service from TMDB, with a free
  API key: the show itself when TVmaze links to it, otherwise the service's
  start page (ADR 0004 amendments, CRI-80, CRI-82)
...
- TVmaze as the data source, and TMDB (free key) for the Swedish service

## Out of scope
Everything tagged MVP or later in `01-prd.md`: the menu for several
services, season level territory ("Not in Sweden yet"),
